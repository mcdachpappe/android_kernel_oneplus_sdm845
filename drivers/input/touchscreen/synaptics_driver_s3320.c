...
static void gesture_judge(struct synaptics_ts_data *ts)
{
	...
	if ((gesture == DouTap && DouTap_gesture)||(gesture == RightVee && RightVee_gesture)
        ...
        ||(gesture == Down2UpSwip && Down2UpSwip_gesture)||(gesture == Sgestrue && Sgestrue_gesture)
        ||(gesture == Wgestrue && Wgestrue_gesture)||(gesture == Mgestrue && Mgestrue_gesture)
...
static void tp_gesture_set_enable(void) {
	...
	TPD_DEBUG("gesture suport LeftVee:%d RightVee:%d DouSwip:%d Circle:%d UpVee:%d DouTap:%d DownVee:%d Left2RightSwip:%d Right2LeftSwip:%d Up2DownSwip:%d Down2UpSwip:%d\n Sgestrue:%d\n Mgestrue:%d\n Wgestrue:%d\n SingleTap:%d\n",
		LeftVee_gesture,RightVee_gesture,DouSwip_gesture,Circle_gesture,UpVee_gesture,DouTap_gesture,DownVee_gesture,Left2RightSwip_gesture,
		Right2LeftSwip_gesture,Up2DownSwip_gesture,Down2UpSwip_gesture,Sgestrue_gesture,Mgestrue_gesture,Wgestrue_gesture,SingleTap_gesture);

	if (DouTap_gesture||Circle_gesture||UpVee_gesture||LeftVee_gesture||RightVee_gesture||DouSwip_gesture
		||Sgestrue_gesture||Mgestrue_gesture||Wgestrue_gesture||DownVee_gesture||Left2RightSwip_gesture
		||Right2LeftSwip_gesture||Up2DownSwip_gesture||Down2UpSwip_gesture||SingleTap_gesture)
	{
		ts->gesture_enable = 1;
	}
	else
				     ...
				     loff_t * ppos)
{
	...
	SingleTap_gesture = (buf[1] & BIT3) ? 1 : 0;	//single tap
